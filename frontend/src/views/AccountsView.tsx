...
function StatCard({
	...
	return (
		...
		>
			...
			<p
				className={`text-3xl font-semibold tracking-tight leading-none ${danger ? "text-red-600" : "text-foreground"}`}
			>
				{value}
			</p>
		...
		<div className="flex flex-col gap-6 p-6 md:p-8 max-w-350">
			...
			<div>
				...
				<p className="mt-0.5 text-xs text-muted-foreground">
					Enter an account ID to view its full transaction history and
					risk profile.
				</p>
			</div>
			...
			{searching && (
				<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
					{[...Array(4)].map((_, i) => (
						<Skeleton
							key={i}
							className="h-28 rounded-xl bg-zinc-100"
						/>
					))}
				</div>
			)}
				...
				<>
							...
							{profile.open_alerts > 0 && (
								<Badge className="rounded-md bg-orange-50 border border-orange-200 text-orange-700 text-[11px] font-medium">
									<FiAlertTriangle
										...
										className="mr-1"
									/>
									{profile.open_alerts} open alert
									{profile.open_alerts !== 1 ? "s" : ""}
								</Badge>
							)}
							{profile.fraud_rate_percent > 5 && (
								<Badge
									variant="outline"
									className="rounded-md border-red-200 bg-red-50 text-[11px] font-medium text-red-700"
								>
					...
					<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
						...
						<StatCard
							label="Avg Amount"
							value={
								profile.avg_amount != null
									? `$${profile.avg_amount.toFixed(2)}`
									: "--"
							}
							sub={
								profile.max_amount != null
									? `Max: $${profile.max_amount.toFixed(2)}`
									: undefined
							}
						...
						<StatCard
							...
							value={
								profile.avg_risk_score != null
									? profile.avg_risk_score.toFixed(4)
									: "--"
							}
							sub={`${profile.total_alerts} total alerts`}
							danger={(profile.avg_risk_score ?? 0) > 0.65}
							accentColor={
								(profile.avg_risk_score ?? 0) > 0.65
									? "oklch(0.65 0.18 50)"
									: "oklch(0.6 0.12 280)"
							}
						/>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="mt-0.5 text-xs text-muted-foreground">
								Last active:{" "}
								{formatTimestamp(profile.last_transaction)}
							</p>
						</div>
								...
								<TableBody>
									{transactions.length === 0 ? (
										...
										transactions.map((tx) => (
											...
											>
												<TableCell className="mono pl-5 text-[11px] text-muted-foreground">
													#{tx.id}
												</TableCell>
